...
export default function ShiftStatusCard({ slot, onEndShift, loading }: ShiftStatusCardProps) {
  ...
  if (slot.closeTime) {
    return (
      <div className="p-4 rounded-lg border-2 border-foreground/20 bg-muted animate-in fade-in slide-in-from-top-2">
  ...
  return (
    <div className="p-4 rounded-lg border-2 border-foreground bg-background animate-in fade-in slide-in-from-top-2">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        ...
        <button
          onClick={onEndShift}
          disabled={loading}
          className="h-11 px-6 rounded-md bg-foreground text-background hover:bg-foreground/90 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-colors inline-flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
